...
*/
void hanoi(int discSize, int source, int dest, int spare)
{
  //This is our standard termination case. We get to here when we are operating on the 
  //smallest possible disk.
  if(discSize == -1)
  {
  	cout << "No discs to move, bruh." << endl;
  	
  }
  else if(discSize == 0)
	{
		std::cout << "Move disc from peg " << source << " to peg " << dest << endl;
	}
	else
	{
...
{
  int disc_count;
  cout << "Please enter number of discs to move: " << endl << "     ----> ";
  cin >> disc_count;
  //Move all 3 disks from peg 0 to peg 1 using peg 2 as a temporary.
  hanoi(disc_count-1, 1, 3, 2);
  return 0;
}
